...
const ApplyLeave = () => {
    ...
    const approvedLeaveCount = leaveTypes.map(type => {
        const approvedCount = approvedLeaves.filter(item => item.leave_type === type.leave_type_name && item.status === 'approved').length; // Use approvedLeaves instead of data
        return {
                            ...
                            <Card className="shadow-lg rounded bg-light registration-card " >
                                ...
                                >
                                    <div className="mb-3">
                                        ...
                                        >
                                            ...
                                            {leaveTypes.map((item) => (
                                                <option key={item.leave_type_name} value={item.leave_type_name}>
                                ...
                                {leave && (
                                    <div className="mt-3 text-center">
                                        <p className="text-darkblue">Approved Leaves: {approvedLeaveCount.find(item => item.leave_type_name === leave)?.approved || 0} / {leaveLimits[leave] || 0}</p>
                                        <p className="text-darkblue">
                                            {approvedLeaveCount.find(item => item.leave_type_name === leave)?.approved >= (leaveLimits[leave] || 0)
                                                ? `You have reached the limit for ${leave}.`
                                                : `You can still apply for ${leaveLimits[leave] - (approvedLeaveCount.find(item => item.leave_type_name === leave)?.approved || 0)} more ${leave}(s).`}
                                        </p>
                                    </div>
                                )}
                                </form>
